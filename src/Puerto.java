...
public class Puerto {
    ...
    public String buscaContenedoresBase(int prioridad){
        String s = new String();
        boolean buscar = false;
        for (int k = 0; k < hubs.length; k++) {
                for (int j = 0; j < hubs[k].m[9].length; j++) {
                    if(hubs[k].m[9][j] != null){
                        if( hubs[k].m[9][j].getPrioridad() == prioridad) {
                            s += "\n" + "ID: " + hubs[k].m[9][j].getId() + "\n" + "Remitente: " + hubs[k].m[9][j].getEmisor() + "\n" + "Peso: " + hubs[k].m[9][j].getPeso() + "\n" + "Chequeo: " + hubs[k].m[9][j].isInspeccionado();
                            buscar = true;
                        }
                    }
                }
        ...
        if(!buscar){
            return "No se ha encontrado ningun contenedor";
        }
        return s;
    }
}
